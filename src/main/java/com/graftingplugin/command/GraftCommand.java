...
public final class GraftCommand implements CommandExecutor, TabCompleter {
    ...
    private void handleConcept(CommandSender sender, String[] args) {
        ...
        CastSession session = plugin.castSessionManager().session(player.getUniqueId());
        session.setPendingConcept(args[1].toLowerCase(Locale.ROOT));
        plugin.messages().send(player, "concept-armed", "concept", args[1]);
    }
    ...
    private void handleInspect(CommandSender sender) {
        ...
        CastSession session = plugin.castSessionManager().session(player.getUniqueId());
        String concept = session.pendingConcept() == null ? "none" : session.pendingConcept();
        player.sendMessage("Mode: " + session.family().displayName());
        player.sendMessage("Pending concept: " + concept);
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) {
            return filter(List.of("mode", "concept", "inspect", "clear", "active", "givefocus", "reload"), args[0]);
        }
